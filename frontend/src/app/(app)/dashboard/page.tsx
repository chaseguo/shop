...
import {
  TrendingUp, TrendingDown, Eye, AlertCircle, FileText,
...
const kpis = [
  {
    label: "LRCI 指数",     value: "47.2",  change: "+3.1",  up: true,
    color: "text-blue-500", bgColor: "bg-blue-500/8",
    icon: TrendingUp, trend: gen(45),
  },
  {
    label: "推荐率",        value: "35%",   change: "+2%",   up: true,
    color: "text-emerald-500", bgColor: "bg-emerald-500/8",
    icon: Eye, trend: gen(33),
  },
  {
    label: "负面占比",      value: "18%",   change: "−1.5%", up: false,
    color: "text-red-500",  bgColor: "bg-red-500/8",
    icon: AlertCircle, trend: gen(20).map(d => ({ ...d, value: Math.max(12, d.value) })),
  },
  {
    label: "可引用条目",    value: "142",   change: "+12",   up: true,
    color: "text-violet-500", bgColor: "bg-violet-500/8",
    icon: FileText, trend: gen(130),
  },
];
...
const priorityColors: Record<string, string> = { high: "bg-red-500", medium: "bg-amber-500", low: "bg-slate-400" };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between rounded-2xl border border-blue-200 bg-blue-50 dark:border-blue-900/60 dark:bg-blue-950/30 px-5 py-3">
        ...
        <Button variant="ghost" size="sm" className="text-blue-600 dark:text-blue-400 h-7 rounded-lg text-xs" asChild>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        {kpis.map((k) => {
          const Icon = k.icon;
          return (
            <div key={k.label} className="card-lift rounded-2xl border bg-card p-4 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <div className={cn("rounded-xl p-2", k.bgColor)}>
                  <Icon className={cn("h-4 w-4", k.color)} />
                </div>
                <span className={cn(
                  "flex items-center gap-0.5 text-xs font-semibold",
                  k.up ? "text-emerald-600 dark:text-emerald-400" : "text-red-500 dark:text-red-400"
                )}>
              ...
              <div className="text-xs text-muted-foreground mt-0.5 mb-2">{k.label} · 较上周</div>
              <TrendLine data={k.trend} color={k.color.replace("text-", "").replace("-500", "")} height={48} />
            </div>
          );
        })}
      </div>
      ...
      <div className="grid grid-cols-12 gap-4">
        {/* LRCI gauge */}
        <div className="col-span-12 lg:col-span-3 rounded-2xl border bg-card p-4 shadow-sm">
          ...
          <div className="space-y-1 mt-1">
            {[
              { label: "推荐率 × 0.35",     val: "35%" },
              { label: "Top1 占比 × 0.25",  val: "18%" },
              { label: "正向提及 × 0.20",   val: "42%" },
              { label: "负面提及 × 0.20",   val: "18%" },
            ].map(r => (
        ...
        {/* Brand bar */}
        <div className="col-span-12 lg:col-span-4 rounded-2xl border bg-card p-4 shadow-sm">
        ...
        {/* Radar */}
        <div className="col-span-12 lg:col-span-5 rounded-2xl border bg-card p-4 shadow-sm">
          <div className="flex items-baseline justify-between">
            <div className="text-sm font-semibold text-foreground">竞争力雷达对比</div>
            <span className="text-xs text-muted-foreground">综合三模型 · 五维</span>
          </div>
          <div className="text-xs text-muted-foreground mb-1">度小满 vs 奇富借条 vs 洋钱罐</div>
          <ModelRadar height={220} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-12 gap-4">
        {/* Bubble */}
        <div className="col-span-12 lg:col-span-6 rounded-2xl border bg-card p-4 shadow-sm">
          ...
          <p className="text-xs text-muted-foreground mb-2">气泡大小 = 严重度 | X = 影响分 | Y = 提及次数</p>
        ...
        <div className="col-span-12 lg:col-span-6 rounded-2xl border bg-card p-4 shadow-sm">
          ...
          <div className="space-y-2">
            {tasks.map((t) => {
              ...
              return (
                <div key={t.id} className="flex items-center gap-3 rounded-xl border border-border/60 bg-muted/30 px-3 py-2.5 hover:border-border transition-colors">
          ...
          <div className="mt-4 pt-4 border-t border-border/60">
            ...
            <div className="flex gap-4 mt-2">
              {[{ l: "已完成", v: 8, c: "text-emerald-600 dark:text-emerald-400" }, { l: "进行中", v: 3, c: "text-blue-500" }, { l: "待处理", v: 5, c: "text-amber-500" }].map(s => (
                <div key={s.l} className="flex items-center gap-1.5 text-xs">
                  <span className={cn("font-semibold", s.c)}>{s.v}</span>
